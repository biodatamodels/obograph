...
from obographs.obograph import Node
            ...
            for fname in files:
                ...
                try:
                    if fname.endswith('.yaml'):
                        nyaml += 1
                        o: Node = yaml_loader.load(full_fname, Node)
                        pyaml += 1
                    elif fname.endswith('.json'):
                        njson += 1
                        o: Node = json_loader.load(full_fname, Node)
                        pjson += 1
                    elif fname.endswith('.ttl'):
                        nttl += 1
                        o: Node = rdf_loader.load(full_fname, Node)
                        pttl += 1
